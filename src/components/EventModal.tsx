...
interface EventModalProps {
  ...
  onUpdateEvent?: (eventId: string, updates: Partial<MarketEvent>) => void;
}

type TabKey = 'info' | 'schedule';

export default function EventModal({ event, onClose, onUpdateLinks, onUpdateAIResult, onClearAIResult, onDeleteEvent, onUpdateEvent }: EventModalProps) {
  ...
  const [editAttendees, setEditAttendees] = useState(event.attendees?.toString() || '');
  ...
  const handleSaveEdit = () => {
    onUpdateEvent?.(event.id, {
      ...
      attendees: editAttendees ? parseInt(editAttendees) : undefined,
    });
    setIsEditing(false);
  };
  const handleCancelEdit = () => {
    ...
    setEditAttendees(event.attendees?.toString() || '');
    setIsEditing(false);
  };
  const handleDelete = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(15,37,59,0.35)', backdropFilter: 'blur(12px)' }} onClick={onClose}>
        ...
        <div className="shrink-0">
          ...
          <div className="relative px-6 pt-5 pb-4" style={{ background: `linear-gradient(135deg, ${scaleConfig.bgColor} 0%, rgba(255,255,255,0.9) 60%)` }}>
            <div className="absolute top-4 right-4 flex items-center gap-2">
              {!isEditing && (
                <>
                  <button onClick={() => setIsEditing(true)} className="w-8 h-8 flex items-center justify-center rounded-full text-xs font-semibold" style={{ background: '#003A70', color: '#fff' }}>编辑</button>
                  <button onClick={() => setShowDeleteConfirm(true)} className="w-8 h-8 flex items-center justify-center rounded-full" style={{ background: '#D93A3A', color: '#fff' }}><Trash2 className="w-4 h-4" /></button>
                </>
              )}
              <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-full" style={{ background: '#fff', border: '1px solid #D9D9D6', color: '#28334A' }}>
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            {isEditing ? (
              <div className="space-y-3 pr-24">
                <input value={editTitle} onChange={e => setEditTitle(e.target.value)} className="w-full text-sm px-3 py-2 rounded-lg border" style={{ borderColor: '#D9D9D6' }} placeholder="活动名称" />
                <input type="date" value={editDate} onChange={e => setEditDate(e.target.value)} className="w-full text-sm px-3 py-2 rounded-lg border" style={{ borderColor: '#D9D9D6' }} />
                <input value={editLocation} onChange={e => setEditLocation(e.target.value)} className="w-full text-sm px-3 py-2 rounded-lg border" style={{ borderColor: '#D9D9D6' }} placeholder="地点" />
                <input value={editAttendees} onChange={e => setEditAttendees(e.target.value)} className="w-full text-sm px-3 py-2 rounded-lg border" style={{ borderColor: '#D9D9D6' }} placeholder="参会人数" type="number" />
                <textarea value={editDescription} onChange={e => setEditDescription(e.target.value)} className="w-full text-sm px-3 py-2 rounded-lg border" style={{ borderColor: '#D9D9D6' }} placeholder="描述" rows={2} />
                <div className="flex gap-2">
                  <button onClick={handleSaveEdit} className="px-4 py-2 rounded-lg text-xs font-semibold text-white" style={{ background: '#007A80' }}>保存</button>
                  <button onClick={handleCancelEdit} className="px-4 py-2 rounded-lg text-xs font-semibold" style={{ background: '#F5F5F3', color: '#96A3AD' }}>取消</button>
                </div>
              </div>
            ) : (
              <h3 className="text-lg font-bold leading-snug pr-24" style={{ color: '#0F253B' }}>{event.title}</h3>
            )}
          </div>
      ...
      {/* ===== Delete Confirm ===== */}
      {showDeleteConfirm && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4" style={{ background: 'rgba(15,37,59,0.5)', backdropFilter: 'blur(8px)' }} onClick={() => setShowDeleteConfirm(false)}>
          <div className="w-full max-w-sm rounded-2xl p-6" style={{ background: '#fff', boxShadow: '0 24px 80px rgba(15,37,59,0.2)' }} onClick={e => e.stopPropagation()}>
            <h4 className="text-base font-bold mb-2" style={{ color: '#0F253B' }}>确认删除</h4>
            <p className="text-sm mb-6" style={{ color: '#96A3AD' }}>确定要删除「{event.title}」吗？此操作不可恢复。</p>
            <div className="flex gap-3">
              <button onClick={handleDelete} className="flex-1 py-2.5 rounded-xl text-sm font-semibold text-white" style={{ background: '#D93A3A' }}>确认删除</button>
              <button onClick={() => setShowDeleteConfirm(false)} className="flex-1 py-2.5 rounded-xl text-sm font-semibold" style={{ background: '#F5F5F3', color: '#96A3AD' }}>取消</button>
            </div>
          </div>
        </div>
      )}
