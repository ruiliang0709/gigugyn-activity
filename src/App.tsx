...
function App() {
  ...
  const handleDeleteEvent = useCallback((eventId: string) => {
    const base = pendingEvents !== null ? pendingEvents : localEvents;
    const updated = base.filter(e => e.id !== eventId);
    if (pendingEvents !== null) {
      setPendingEvents(updated);
    } else {
      setLocalEvents(updated);
  ...
  const handleUpdateEvent = useCallback((eventId: string, updates: Partial<MarketEvent>) => {
    const base = pendingEvents !== null ? pendingEvents : localEvents;
    const updated = base.map(e => e.id === eventId ? { ...e, ...updates } : e);
    if (pendingEvents !== null) {
      setPendingEvents(updated);
    } else {
      setLocalEvents(updated);
    ...
    if (pendingEvents === null) {
      fetch('/api/events/save', {
        ...
        if (data.count > 0) utils.event.list.invalidate();
      }).catch(err => console.error('[updateEvent] Error:', err));
    }
  }, [pendingEvents, localEvents, utils]);
